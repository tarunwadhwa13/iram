use bcrypt;
use crate::errors::{ HashingModuleError, AuthenticationError };
...
lazy_static! {
    pub static ref PRIVATE_PEM_FILE_CONTENT: String = fs::read_to_string("/Users/tarunw/.ssh/id_rsa").unwrap().parse().expect("Failed to load private pem file");
    pub static ref PUBLIC_PEM_FILE_CONTENT: String = fs::read_to_string("/Users/tarunw/.ssh/id_rsa.pub").unwrap().parse().expect("Failed to load pub key file");
}
...
pub fn generate_jwt_token(user: AppUser) -> Result<String, AuthenticationError> {
    let key_pair = match RS384KeyPair::from_pem(&PRIVATE_PEM_FILE_CONTENT) {
        ...
        Err(e) => {
            return Err(AuthenticationError(e.to_string()))
        }
    };

    let claims = Claims::create(Duration::from_hours(8)).with_issuer("IRAM").with_subject("Iram Access");
