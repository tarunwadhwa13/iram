pub mod zabbix {
    ...
    use crate::errors::{ZabbixError, UnsupportedError};
    ...
    impl AlertSource for ZabbixHandler {
        ...
        fn get_active_alerts(&mut self) -> Result<AlertList, Box<dyn Error>> {
            ...
            if !zbx_response.is_array() {
                return Err(Box::new(ZabbixError("Expected active triggers response to be array.".to_string())));
            };
    ...
    fn get_zbx_authkey(zbx: &ZabbixHandler, check_if_valid: bool) -> Result<AuthKey, Box<dyn Error>> {
        ...
        match auth_mechanism.as_str() {
            ...
            _ => {
                return Err(Box::new(UnsupportedError("This auth mechanism is not supported".to_string())))
            }
    ...
    fn query_zabbix(zbx: &ZabbixHandler, payload: jsonValue) -> Result<jsonValue, Box<dyn Error>> {
        ...
        let response = client.post(uri).header("Content-Type", "application/json-rpc").json(&payload).send();
        ...
        match parsed_response.result {
            jsonValue::Null => Err(Box::new(ZabbixError("Error Querying Zabbix - No Output received".to_string()))),
